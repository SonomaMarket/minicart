...
const FreeShipping = () => {
  ...
  useEffect(() => {
    ...
    const total = items.reduce(
      (acc: number, item: { priceDefinition: { total: number } }) =>
        acc + item.priceDefinition?.total / 100,
      0
    )
    setTotalPrice(total)
    setPercent((total * 100) / freeShippingPrice)
  }, [items, freeShippingPrice])

  if (!items.length) return null

  if (!freeShippingPrice || isPreSale(items)) return null
    ...
    return (
      <div className={styles.freeShipping}>
        <div className={styles.freeShippingText}>
          Você ganhou{' '}
          <strong className={styles.freeShippingHighlight}>
            frete grátis!
          </strong>
        </div>
        <input
  ...
  return (
    <div className={styles.freeShipping}>
      <div className={styles.freeShippingText}>
        ...
        <strong className={styles.freeShippingHighlight}>
          R$
          {(Number(freeShippingPrice) - Number(totalPrice))
            .toFixed(2)
            .replace('.', ',')}
        </strong>{' '}
        para você ganhar{' '}
      ...
      <input
        ...
        className={styles.freeShippingRange}
        style={{
          background: `linear-gradient(to right, #A72543 0%, #A72543 ${percent}%,
          #fff ${percent}%, white 100%)`,
        }}
        disabled
      />
    </div>
...
function getFreeShippingAmount() {
  const getUfAndCity = JSON.parse(
    window.localStorage.getItem('userAdress') ??
      '{"uf":"N/A","localidade":"N/A"}'
  )
